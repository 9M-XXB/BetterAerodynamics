...
This mod provides a basic aerodynamic implementation in Minecraft for elytra. In this mod, it is assumed that elytra has an airfoil of NACA 2412, but the designation can be customized in the settings screen (press **O**) — the built-in NACA four-digit profile builder regenerates the flight model live. By reading the player's speed in real-time, the mod is able to calculate the boundary layer on the elytra and thus use panel method and 3D wing correction to solve for lift and drag. Turning is modeled with banked lift as well: looking to one side banks the elytra, tilting the lift vector so its horizontal component curves the flight path — like a real glider, a sustained turn sinks unless you pull up, and pulling too hard in a turn can stall. The goal is to provide a more realistic flying experience.
